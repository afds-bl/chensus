...
Suppose we want to estimate the population in the `nhanes` dataset by gender and birth country We can use the main analysis function `chensus::se_total()`:
...
```
We could also estimate population in parallel groups (without combining the variable levels):
...
```
and by birth country interview language:

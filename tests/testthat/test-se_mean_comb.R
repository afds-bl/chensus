test_that("se_mean_ogd returns correct structure and content", {
  df <- nhanes
  result <- se_mean_ogd(df, variable = annual_household_income, strata = strata, weight = weights, gender, birth_country)
...
test_that("se_mean_ogd uses default strata and handles numeric variables", {
  df <- tibble(
  ...
  result <- se_mean_ogd(data = df, variable = size, weight = weight, size, alpha = 0.1)

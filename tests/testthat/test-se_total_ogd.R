test_that("se_total_ogd returns correct structure and content", {
  df <- nhanes
  result <- se_total_ogd(df, strata = strata, weight = weights, gender, birth_country)
...
test_that("se_total_ogd uses default strata and handles numeric variables", {
  df <- tibble(
  ...
  result <- se_total_ogd(data = df, weight = weight, size, alpha = 0.1)

test_that("stratum size 1 works", {
  ...
  expect_equal(mzmv_mean(data = nhanes_na |> filter(is.na(household_size)),
...
test_that("stratum size > 1 works", {
  ...
  expect_equal(mzmv_mean(data = nhanes, variable = "household_size", weight = "weights"),
               out)
})

test_that("mzmv_mean_map equivalent to mzmv_mean with no conditions", {
  out1 <- mzmv_mean(data = nhanes, variable = "household_size", weight = "weights") |>
    as_tibble()
  out2 <- mzmv_mean_map(data = nhanes, variable = "household_size", weight = "weights") |>
